...
public class RoundRobin implements Runnable {
	...
	public void doCPUCycle() {
		...
		}else if(cpu.cpuStack.get(0).IOburstTime.isEmpty()&&cpu.currentBurst==0) {
			LogArea.log("Process: "+cpu.cpuStack.get(0).getName()+" was added to Finished Queue", systemTime);
			cpu.cpuStack.get(0).setFinishTime(systemTime);
			if(!finishedQueue.contains(cpu.cpuStack.get(0))) {
				FinishedArea.addToFinishedTextArea(cpu.cpuStack.get(0).toString());
		...
		}else if(quantumTimer == GUI.quantumTime) {
			LogArea.log("Process: "+cpu.cpuStack.get(0).getName()+" was added back to Ready Queue. Quantum time reached.", systemTime);
...
public void doIOCycle() {
	...
	public RoundRobin(MainGUI main, LogArea Log, FinishedArea fin ) {
		this.GUI = main;
		this.LogArea = Log;
