...
public class PriorityScheduling implements Runnable {
	...
	public int indexSmallestPriInReadyQueue() {
		...
		for(int i = 0; i < readyQueue.size(); i++) {
			if(min > readyQueue.get(i).getPriority()) {
				min = readyQueue.get(i).getPriority();
				index = i;
	...
	public void swapHigherPriority() {
		if(!cpu.cpuStack.isEmpty() && !readyQueue.isEmpty() ) {
			if(cpu.cpuStack.get(0).getPriority() > readyQueue.get(indexSmallestPriInReadyQueue()).getPriority()) {
				cpu.cpuStack.get(0).CPUburstTime.set(0,cpu.currentBurst);
				LogArea.log("Process: "+cpu.cpuStack.get(0).getName()+" was preempted to Ready Queue", systemTime);
				readyQueue.add(cpu.returnProcess());
				cpu.addToStack(readyQueue.get(indexSmallestPriInReadyQueue()));
				readyQueue.remove(indexSmallestPriInReadyQueue());
		...
		if(cpu.currentBurst == 0 && !cpu.cpuStack.isEmpty()) {
			...
			for(int i = 0; i <= cpu.cpuStack.size()-1; i++) {
				if(cpu.cpuStack.get(i).IOburstTime.isEmpty()) {
					...
					cpu.cpuStack.get(0).setTurnAroundTime(cpu.cpuStack.get(0).getTotalCPUWaitTime()+cpu.cpuStack.get(0).getTotalCPUBurstTime());
...
public void doIOCycle() {
	...
	public void run() {
		...
		while(GUI.isSimulating.get()) {
			...
			if(GUI.stopSimulating.get()) {LogArea.log("Simulation Ended", systemTime);
			end();
			isRunning.set(false);
			break;
			}
			if(readyQueue.isEmpty() && waitingQueue.isEmpty() && cpu.cpuStack.isEmpty() && io.ioStack.isEmpty()) {GUI.isSimulating.set(false);
			LogArea.log("Simulation Ended", systemTime); 
			systemTime = 0; 
			break; 
			}		
		}
	...
	public PriorityScheduling(MainGUI main, LogArea Log, FinishedArea fin) {
		this.GUI = main;
		this.LogArea = Log;
