...
public class ShortestJobFirst implements Runnable {
	...
	class SortByBurstTimes implements Comparator<Process> {
		 
	    public int compare(Process p1, Process p2)
	    {
	        
	 
	       return p1.CPUburstTime.get(0) - p2.CPUburstTime.get(0);
	...
	public void updateReadyQueueTextArea(){
		...
		if(readyQueue.size() >= 2) {
			Collections.sort(readyQueue,new SortByBurstTimes());
		}
		for(int i=0; i<=readyQueue.size()-1; i++) {
	...
	public Process selectTheShortestBurst() {
        LinkedList<Integer> burstTimes = new LinkedList<>();
        if(!readyQueue.isEmpty()){
            for (int i = 0; i < readyQueue.size(); i++) {
                burstTimes.add(readyQueue.get(i).CPUburstTime.get(0));
            }
            Collections.sort(burstTimes);
            
            for (int i = 0; i < readyQueue.size(); i++) {
                if (burstTimes.get(0)== readyQueue.get(i).CPUburstTime.get(0)) {
	...
	public void doCPUCycle() {
		...
		if(cpu.cpuStack.isEmpty() && !readyQueue.isEmpty()) {
			cpu.addToStack(selectTheShortestBurst());
...
public void doIOCycle() {
	...
	public ShortestJobFirst(MainGUI main, LogArea Log, FinishedArea fin) {
		this.GUI = main;
		this.LogArea = Log;
